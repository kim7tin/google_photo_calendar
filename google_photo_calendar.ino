...
#define GOOGLE_PHOTO_SHARE_LINK "https://photos.app.goo.gl/xxx"
...
#if defined(ESP32)
#define GMT_OFFSET_SEC 25200L  // Timezone +0800
...
void drawMyBitmap(int16_t x, int16_t y,
...
void printTime()
{
  ...
  int hour = tm->tm_hour;
  int min = tm->tm_min;
  int8_t day = tm->tm_mday;
  int8_t wday = tm->tm_wday;
  int8_t mon = tm->tm_mon + 1;
  int16_t year = tm->tm_year + 1900;


  convertSolar2Lunar(day, mon, year, &ngayAm, &thangAm, &namAm);
  ...
  gfx->setCursor(220 - (2 * 5 * 8), 7);
  gfx->printf("Thang %02d", mon);

  gfx->setTextSize(7);
  gfx->setCursor((w - (7 * 5 * (day > 9 ? 2 : 1)) - 2) / 2, 48);
  gfx->setTextColor(WHITE);
  gfx->printf("%d", day);

  gfx->setTextSize(7);
  gfx->setCursor((w - (7 * 5 * (day > 9 ? 2 : 1)) + 2) / 2, 52);
  gfx->setTextColor(WHITE);
  gfx->printf("%d", day);
  ...
  gfx->setTextSize(7);
  gfx->setCursor((w - (7 * 5 * (day > 9 ? 2 : 1))) / 2, 50);
  gfx->setTextColor(BLUE);
  gfx->printf("%d", day);

  gfx->setTextSize(2);
  gfx->setCursor((w - (2 * 5 * (dayInVietNam[wday].length())) - 2) / 2, 103);
  gfx->setTextColor(WHITE);
  gfx->print(dayInVietNam[wday]);

  gfx->setTextSize(2);
  gfx->setCursor((w - (2 * 5 * (dayInVietNam[wday].length())) + 2) / 2, 107);
  gfx->setTextColor(WHITE);
  gfx->print(dayInVietNam[wday]);
  ...
  gfx->setTextSize(2);
  gfx->setCursor((w - (2 * 5 * (dayInVietNam[wday].length()))) / 2, 105);
  gfx->setTextColor(BLUE);
  gfx->print(dayInVietNam[wday]);

  drawMyBitmap(210 - (3 * 5 * 5) - (32 + 10), 279, congiap_bmp[namAm % 12], 32, 32, RED);
  gfx->setTextSize(3);
  gfx->setCursor(210 - (3 * 5 * 5), 280);
  gfx->setTextColor(BLUE);
  gfx->printf("%02d-%02d", ngayAm, thangAm);

  gfx->setTextSize(1);
  gfx->setCursor(210 - (3 * 5 * 5), 305);
  gfx->setTextColor(BLUE);
  gfx->print("nam ");
  gfx->print(can[namAm %10]);
  gfx->print(" ");
  gfx->print(chi[namAm %12]);
  ...
  else
  {
    ...
    if (!photoCount)
    {
      ...
      else
      {
        ...
        char filename[16];
        sprintf(filename, "/kenhtaymay.jpg");
...
void addWhiteAlpha(int16_t x, int16_t y, uint8_t *bitmap, int16_t w, int16_t h, uint8_t percentage)
{
  int32_t offset = 0;
  for (int16_t j = 0; j < h; j++, y++)
  {
    for (int16_t i = 0; i < w; i++)
    {
      bitmap[offset] = (bitmap[offset] * (100 - percentage) / 100) + (255 * percentage / 100);
      bitmap[offset + 1] = (bitmap[offset + 1] * (100 - percentage) / 100) + (255 * percentage / 100);
      bitmap[offset + 2] = (bitmap[offset + 2] * (100 - percentage) / 100) + (255 * percentage / 100);
      offset += 3;
    }
  }
}
...
static bool tft_writer(void *arg, uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint8_t *data)
{
  if (data)
  {
    ...
    if ((y >= 0 && y < 30) || (y >= 270 && y < 320)) {
      addWhiteAlpha(x, y, data, w, h, 70);
    }
